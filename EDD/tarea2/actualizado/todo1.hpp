...
public:
    ...
    for (unsigned i = listSize - 1; i > 0; i--) {
        int j = rand() % (i + 1);
...
public:
    ...
    bool canAttack(int& playerEnergy, int& playerMana){
        ...
        if (type == 2 && ammo == 0){
            if (playerEnergy < reloadCost + energyCost){
                cout << "Not enough energy to reload or attack. ";
                return false;
            }
            cout << "Weapon needs to reload. "; 
            return false; 
        }
...
private:
    ...
    Damage attack; // Ataque base del jugador
...
public:
    ...
    void effect(int code, int sum, float mult) {
        switch (code) {
            ...
            case 3: accuracy *= mult; if(accuracy < 0) accuracy = 0; if(accuracy > 1) accuracy = 1; break; 
        }
    }
};
...
private:
    int enemyListSize;
    Enemy* enemys; // Array dinámico de enemigos
...
public:
    ...
    ~Combat() {
        delete[] enemys; // Liberar memoria del array de enemigos
    }
    ...
    void selectRandomEnemiesFromList(list<Enemy>& sourceList, Enemy* targetArray, int& currentIndex, int maxToSelect) {
        ...
        list<Enemy> tempList; // Copia temporal
        ...
        while (selectedCount < maxToSelect && currentIndex < enemyListSize && currentIndex < 4 && tempList.currPos() < tempList.length()) {
            if (tempList.currPos() >= 0 && tempList.currPos() < tempList.length()) { // Chequeo de validez del cursor
                 targetArray[currentIndex] = tempList.getValue();
                 targetArray[currentIndex].resetHealth(); // Asegurar que los enemigos spawnean con vida completa
                 currentIndex++;
                 selectedCount++;
            }
            tempList.next();
    ...
    void setEnemyList() {
        ...
        delete[] enemys; // Borrar enemigos anteriores si existen
        enemys = nullptr;
        
        // Determinar número de enemigos (1-4)
        enemyListSize = (rand() % 4) + 1;
        enemys = new Enemy[enemyListSize]; // Crear nuevo array
        
        int currentEnemyIndex = 0;
        int enemiesToSpawn = enemyListSize;
            ...
            switch (category) {
                case 0:
                    if (enemigosBase.length() > 0)
                        selectRandomEnemiesFromList(enemigosBase, enemys, currentEnemyIndex, numFromCategory);
                    break;
                case 1:
                    if (enemigosDefensivos.length() > 0)
                        selectRandomEnemiesFromList(enemigosDefensivos, enemys, currentEnemyIndex, numFromCategory);
                    break;
                case 2:
                    if (enemigosFuertes.length() > 0)
                        selectRandomEnemiesFromList(enemigosFuertes, enemys, currentEnemyIndex, numFromCategory);
                    break;
            }
            if (currentEnemyIndex == enemiesToSpawn) break; // Salir si ya se generaron suficientes
        }
        // Rellenar si no se generaron suficientes (por ejemplo, si se eligieron categorías vacías)
        while(currentEnemyIndex < enemyListSize) {
            if(enemigosBase.length() > 0) selectRandomEnemiesFromList(enemigosBase, enemys, currentEnemyIndex, 1);
            else if(enemigosDefensivos.length() > 0) selectRandomEnemiesFromList(enemigosDefensivos, enemys, currentEnemyIndex, 1);
            else if(enemigosFuertes.length() > 0) selectRandomEnemiesFromList(enemigosFuertes, enemys, currentEnemyIndex, 1);
            else break; // No hay más enemigos para generar
        }


        cout << "\n=== ENEMIGOS APARECEN ===" << endl;
        for (int i = 0; i < currentEnemyIndex; i++) { // Usar currentEnemyIndex como límite real de enemigos generados
            cout << (i+1) << ". " << enemys[i].getName() 
                 << " (HP: " << enemys[i].getHealth() << "/" << enemys[i].getBaseHealth() << ")" << endl;
        }
        enemyListSize = currentEnemyIndex; 
        cout << "=========================" << endl;
    }

    bool allEnemiesDead() { 
        if (enemyListSize == 0 && enemys == nullptr) return true;
        if (enemys == nullptr) return true; // No hay enemigos
    ...
    void applyDamageToEnemy(Enemy& enemy, Damage weaponDmg, Damage playerBaseDmg, float weaponAcc, float playerAcc) {
        if (static_cast<float>(rand()) / RAND_MAX > weaponAcc * playerAcc) {
            cout << "You missed the attack on " << enemy.getName() << endl;
            return;
        }

        Damage enemyDef = enemy.getDefence();
        Damage totalEffectiveDamage; // Daño que realmente se aplica

        // Calcular daño para cada tipo
        ...
        // Sumar solo daños positivos; el daño negativo es regeneración
        ...
        if (physDmg < 0) totalRegeneration -= physDmg; // - (-ve) = +ve regeneration
        ...
        if (totalDamageApplied > 0) {
            ...
            cout << enemy.getName() << " received " << totalDamageApplied << " damage.";
        }
        if (totalRegeneration > 0) {
            enemy.receiveDamage(-totalRegeneration); // receiveDamage resta, así que pasamos negativo para sumar vida
            cout << enemy.getName() << " regenerated " << totalRegeneration << " health.";
        }
        if (totalDamageApplied == 0 && totalRegeneration == 0 && !(static_cast<float>(rand()) / RAND_MAX > weaponAcc * playerAcc) ) {
             cout << enemy.getName() << " received no damage and no regeneration from the attack.";
        }


        // Mostrar mensaje de efectividad basado en el tipo de daño primario del arma
        // (Esta lógica podría necesitar que Weapon indique su tipo de daño principal)
        ...
        else { cout << endl; } // Si no hay daño positivo, no hay mensaje de efectividad
    }

    void effectivityMessage(float def) {
        if (def <= -1.0f) { cout << " That was critically effective!" << endl; } // Más del 100% de daño extra
        else if (def < 0)  { cout << " That was very effective."   << endl; }
        else if (def == 0) { cout <<                                  endl; } // Daño normal
        else if (def < 0.5f){ cout << " That was not very effective."      << endl; }
        else if (def < 1.0f)  { cout << " That was highly ineffective." << endl; }
        else if (def >= 1.0f) { cout << " Enemy is Inmune to this damage type." << endl; } // 100% o más de resistencia
    }

    void Combating(Player& player) {
        player.startBattle();
        // setEnemyList(); // Mover la generación de enemigos al inicio del combate o playerTurn si es la primera vez
        
        bool firstTurn = true;

        while (true) {
            if (firstTurn) {
                 setEnemyList(); // Generar enemigos para este combate
                 if (enemyListSize == 0) { // No se pudieron generar enemigos
                      cout << "No enemies appeared. Combat ends." << endl;
                      return;
                 }
                 firstTurn = false;
            }
            player.playerTurn(); // Restaurar energía, ganar mana
            ...
            if (allEnemiesDead()) {
                ...
                selectRandomItems(player); // Función global
    ...
    void enemyTurnLogic(Player& player) {
        ...
        for (int i = 0; i < enemyListSize; i++) {
            ...
            cout << enemys[i].getName() << " attempts to attack." << endl;
            timer(1); // Pequeña pausa
            ...
            if (static_cast<float>(rand()) / RAND_MAX > enemys[i].getAccuracy()) {
                ...
                cout << "You received " << damageDealt << " damage from " << enemys[i].getName() << "." << endl;
                anyEnemyAttacked = true;
            }
            timer(1);
        }
        if (!anyEnemyAttacked && !allEnemiesDead()) {
             cout << "The remaining enemies ponder their next move..." << endl;
        }
    ...
    void playerTurnLogic(Player& player) { 
        ...
        while(!turnEnded && player.getEnergy() > 0) { // Permitir acciones mientras haya energía
            cout << "\nChoose action: attack (1-"<<player.AmountWeapons()<<"), reload <slot>, pass (o 'endTurn'): ";
            cin >> input;
            ...
            // Lógica para 'reload <slot>'
            if (input == "reload") {
                int slotToReload;
                cout << "Reload which weapon slot (1-4)? ";
                cin >> slotToReload;
                if (cin.fail() || slotToReload < 1 || slotToReload > 4) {
                    cout << "Invalid slot." << endl; cin.clear(); cin.ignore(10000, '\n'); continue;
                }
                node<Weapon>* weaponNode = player.invWeapon(slotToReload - 1);
                if (weaponNode && weaponNode->data.getType() == 2) {
                    int currentEnergy = player.getEnergy(); // Copia para la función reload
                    if(weaponNode->data.reload(currentEnergy)) {
                        
                         int energyBeforeReload = player.getEnergy();
                         weaponNode->data.reload(energyBeforeReload); 
                         
                } else {
                    cout << "Weapon is not ranged or slot is empty." << endl;
                }
            ...
            try {
                ...
                cout << "Invalid entry. Choose weapon (1-4), reload <slot>, or 'endTurn': ";
                continue;
            }

            if (weaponSlotChoice < 1 || weaponSlotChoice > 4 ) {
            ...
            Weapon currentWeapon = weaponNode1->data; // Copia del arma para trabajar con ella
            
            // Pasar referencias de energía y maná del jugador
            int playerEnergy = player.getEnergy();
            int playerMana = player.getMana();

            if (!currentWeapon.canAttack(playerEnergy, playerMana)) { // canAttack ahora modifica playerEnergy, playerMana
                
                cout << "Choose weapon (1-4) or 'endTurn': "; // Repetir si no se pudo atacar
               
                continue;
            }
            
            weaponNode1->data = currentWeapon; // Actualiza el arma en el inventario del jugador (importante para munición)


            if (currentWeapon.isMultipleShot()) {
                cout << "Using " << currentWeapon.getName() << " against all enemies!" << endl;
                ...
                while (!validTarget && !allEnemiesDead()) {
                    cout << "Choose enemy target (1-" << enemyListSize << ") or 'cancel': ";
                    cin >> input;
                    ...
                    cout << "Attacking " << enemys[targetEnemyIndex].getName() << " with " << currentWeapon.getName() << "!" << endl;
                    for (int i = 0; i < currentWeapon.getBurst(); i++) {
                        applyDamageToEnemy(enemys[targetEnemyIndex], currentWeapon.getAttack(), player.getAttack(), 
                                       currentWeapon.getAccuracy(), player.getAccuracy());
                        if (enemys[targetEnemyIndex].getHealth() <= 0) {
                             cout << enemys[targetEnemyIndex].getName() << " has been defeated!" << endl;
                             break; // Salir del burst si el objetivo murió
                        }
                    }
                    validTarget = true; // Ataque realizado (o intento)
             ...
             if (turnEnded) break; // Salir del bucle de acciones si el combate terminó o se pasó el turno
        } // Fin del bucle de acciones del jugador
    }
};
};
...
struct SalaNode {
    ...
    TipoSala tipo;
    Lista<SalaNode*> siguientesSalas; // Usa la clase 'Lista' definida en "estructuras_datos 1.hpp"
    ...
    switch(tipo) {
        ...
        case COMBATE:
            ...
            // Si se llega aquí, el jugador ganó el combate.
        ...
        case ITEMS:
            ...
            selectRandomItems(jugador); // Usa la función global
            timer(1);
            mostrarOpciones();
            salaActual = elegirSiguienteSala();
            break;
        ...
        case ARMAS:
            ...
            mostrarOpcionesArmas(jugador, weaponPool); // Usa la función global
            timer(1);
            mostrarOpciones();
            salaActual = elegirSiguienteSala();
            break;
    ...
    {
    if (siguientesSalas.getSize() == 0 && tipo != FINAL) { // getSize() es de 'Lista'
        cout << "No hay a dónde ir desde aquí... parece un callejón sin salida." << endl;
        // Podría ser un final implícito o un bug de diseño del mapa.
        return;
    }
    if (tipo == FINAL) return; // No hay opciones después de un final

    cout << "\n--- UBICACIONES DISPONIBLES ---" << endl;
    for (int i = 0; i < siguientesSalas.getSize(); i++) {
        cout << (i + 1) << ". " << siguientesSalas[i]->nombre << endl; // operator[] es de 'Lista'
    }
    cout << "-------------------------------" << endl;
}
    ...
    do {
        ...
        try {
            ...
            opcion = 0; // Forzar reintento
        }
        if (cin.fail()) { cin.clear(); cin.ignore(10000,'\n'); } // Limpiar errores de cin

    } while (opcion < 1 || opcion > siguientesSalas.getSize());
    
    return siguientesSalas[opcion - 1]; // operator[] es de 'Lista'
}
};
...
private:
    SalaNode* salaInicio;
    Lista<SalaNode*> todasLasSalas;  // Usa 'Lista'
...
public:
    SistemaJuego() : salaInicio(nullptr) {
        ...
        if (!game_seed_initialized) {
            srand(static_cast<unsigned>(time(0))); // Semilla global para el juego
    ...
    void inicializarSalas() {
        ...
        for (int i = 0; i < salasCombate.getSize(); i++) todasLasSalas.append(salasCombate[i]);
        for (int i = 0; i < salasItems.getSize(); i++) todasLasSalas.append(salasItems[i]);
        for (int i = 0; i < salasArmas.getSize(); i++) todasLasSalas.append(salasArmas[i]);
        for (int i = 0; i < salasLore.getSize(); i++) todasLasSalas.append(salasLore[i]);
        for (int i = 0; i < salasFinales.getSize(); i++) todasLasSalas.append(salasFinales[i]);
    }
    ...
    void asignarSalasAleatorias(SalaNode* sala) {
        int numConexiones = 2 + (rand() % 2); // 2 o 3 conexiones
        Lista<SalaNode*> candidatas;
        
        for(int i=0; i < todasLasSalas.getSize(); ++i) {
            if (todasLasSalas[i] != sala && todasLasSalas[i]->tipo != INICIO) { // No volver al inicio directamente
                 // Añadir con cierta probabilidad o por tipo
                 if (todasLasSalas[i]->tipo != FINAL || (rand() % 100 < 10)) { // 10% prob de sala final
                      candidatas.append(todasLasSalas[i]);
                 }
            }
        }
        for (int i = 0; i < numConexiones && candidatas.getSize() > 0; i++) {
            ...
            candidatas.removeAt(indiceAleatorio); // removeAt() es de 'Lista'
        }
    }
    ...
    void iniciarJuego(Player& jugador, Combat& combate, list<Weapon>& weaponPool) {
        ...
        while (!juegoTerminado && salaActual != nullptr) {
            ...
            if (juegoTerminado && jugador.getHealth() <= 0) { // Si murió
                ...
                if (respuesta == 's' || respuesta == 'S') {
                    jugador = Player(jugador.getName(), jugador.maxHealth(), jugador.maxEnergy(), jugador.maxMana(), 5, jugador.getAccuracy()); // Reiniciar stats base
                ...
                juegoTerminado = true; // Callejón sin salida
            }
        }
...
const int MAX_ITEMS_DEFINED = 28; // Número de items en createSpecificItem
...
void applySpecialItemEffects(const string& itemName, Player& player) {
    if (itemName == "Mantequilla") {
        ...
        cout << "Tu estilo ha disminuido en 1 punto (pero sigues siendo genial con un balde en la cabeza)." << endl;
    } else if (itemName == "Cafe Doble Extra Cargado") {
        cout << "¡Ahora tienes ansiedad! (Y -25 de vida máxima, pero x3 de energía máxima)" << endl;
    }
}
...
void useItem(Item& item, Player& player ) { 
    ...
    // Manejar efectos especiales primero
    if (item.getName() == "Mantequilla" || item.getName() == "Bendicion de Dios" || 
        item.getName() == "Balde" || item.getName() == "Cafe Doble Extra Cargado") {
        applySpecialItemEffects(item.getName(), player);
    }
    // Aplicar efectos genéricos del item (Cafe Doble Extra Cargado tiene ambos)
    item.applyTo(player);   
...
void selectRandomItems(Player& player) {
    ...
    cout << "========================================" << endl;
    // Mostrar stats ANTES de elegir.
    cout << "\nEstadisticas antes de elegir item:" << endl;
    ...
    int itemsToOffer = std::min(3, MAX_ITEMS_DEFINED); // No ofrecer más de los que hay

    for (int i = 0; i < itemsToOffer; i++) {
        int randomIndex;
        if (MAX_ITEMS_DEFINED == 0) break; // No hay items para ofrecer
        do {
            randomIndex = rand() % MAX_ITEMS_DEFINED;
        } while (usedIndices[randomIndex] && MAX_ITEMS_DEFINED > i); // Evitar bucle infinito si todos los items ya se usaron y MAX_ITEMS_DEFINED <= i
    ...
    Item tempItems[3]; // Para obtener nombres y descripciones
    for (int i = 0; i < itemsToOffer; i++) {
        tempItems[i] = createSpecificItem(selectedIndices[i]);
        cout << "\n[" << (i + 1) << "] " << tempItems[i].getName() << endl;
        showItemDescription(tempItems[i].getName());
    }
    ...
    if (itemsToOffer == 0) {
        cout << "No hay items para elegir esta vez." << endl;
        return;
    }

    cout << "\n========================================" << endl;
    cout << "Ingresa tu eleccion (1-" << itemsToOffer << "): ";
    
    while (!validChoice) {
        cin >> input_str;
        try {
            choice = stoi(input_str);
            if (choice >= 1 && choice <= itemsToOffer) {
                validChoice = true;
            } else {
                cout << "Opcion invalida. Ingresa un numero entre 1 y " << itemsToOffer << ": ";
            }
        } catch (const std::exception& e) {
    ...
    Item chosenItem = tempItems[choice - 1]; // Usar el item ya creado
    cout << "\n========================================" << endl;
    cout << "Has seleccionado: " << chosenItem.getName() << endl;
    cout << "========================================" << endl;
    
    useItem(chosenItem, player); // Pasamos el item y su nombre
...
// ===== FUNCIONES DE ARMAS (DEFINICIONES) =====
...
void mostrarOpcionesArmas(Player& jugador, list<Weapon>& weaponPool){
    ...
    int num_to_offer = std::min(3, weaponPool.length());

    int p_indices[weaponPool.length()];
for(int i=0; i<weaponPool.length(); i++) p_indices[i] = i;
for(int i=weaponPool.length()-1; i>0; i--) {
    int j = rand() % (i+1);
    int temp = p_indices[i];
    p_indices[i] = p_indices[j];
    p_indices[j] = temp;
}
    ...
    for (int i = 0; i < num_to_offer; i++) {
        if (options[i] != nullptr) {
            ...
            cout << "    Precisión: " << options[i]->data.getAccuracy() * 100 << "%, Costo Energía: " << options[i]->data.getEnergyCost() << endl;
        }
    }
    ...
    if (cin.fail()) { cin.clear(); cin.ignore(10000, '\n'); cout << "Selección inválida." << endl; return;}


    if (seleccion == 0) {
        cout << "No has tomado ningún arma." << endl;
        return;
    }
    ...
    if (jugador.AmountWeapons() >= 4 ) {
        ...
        if (respuesta == 's' || respuesta == 'S') {
            ...
            if (cin.fail()) { cin.clear(); cin.ignore(10000, '\n'); cout << "Índice inválido." << endl; return;}


            if (indiceReemplazo >= 1 && indiceReemplazo <= 4) {
                // El nodo del arma elegida (chosenWeaponNode) es el que se va al inventario.
                // No se crea un nuevo nodo. El nodo se "mueve" del pool al jugador.
                jugador.getNewWeapon(indiceReemplazo - 1, chosenWeaponNode); // Los slots son 0-3
                weaponPool.moveToPos(originalIndexOfChosenWeapon);
                weaponPool.erase(); 
                cout << "Arma en posición " << indiceReemplazo << " reemplazada por " << chosenWeaponNode->data.getName() << "." << endl;
            } else {
                cout << "Índice inválido." << endl;
            }
        } else {
            cout << "Operación cancelada. El arma " << chosenWeaponNode->data.getName() << " no fue tomada." << endl;
        }
    } else {
...
void iniciarJuegoCompleto() {
    ...
    // 2. Crear el pool específico para esta partida y barajarlo
    ...
    sistemaJuego.iniciarJuego(jugadorSeleccionado, sistemaComabate, weaponPoolDelJuego);
};
