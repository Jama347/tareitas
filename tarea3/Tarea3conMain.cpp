...
graph* data(string archive, int*& client){
    ...
    while (i < clients) {
            arch >> a;
            client[i] = a;
            Data->info[a-1].estado = 2; //Data->setMark(a,2);
            i++;
    }
...
void solicitarUber(tGrafo& G, int inicio, int destino) {
    ...
    G.info[mejorConductor - 1].estado = 0;    // Ya no esta en su posicion original
    G.info[destino - 1].estado = 2;           // Ahora esta en el destino
...
int main() {
    int* client;
    graph* G = data("data1.txt", client);
    
    if (G == nullptr) {
        cout << "Error al cargar el archivo de datos." << endl;
        return 1;
    }
    ...
    while (true) {
        ...
        // Verificar si el viaje es posible
        int n = G->getNvertices();
        int* dist = new int[n + 1];
        int* padre = new int[n + 1];
        
        // Inicializar padre para reconstruir el camino
        for (int i = 1; i <= n; i++) {
            padre[i] = -1;
        }
        ...
        // Calcular distancia del viaje y construir el camino
        // Reinicializar para Dijkstra desde el inicio
        for (int i = 1; i <= n; i++) {
            dist[i] = INFINITO;
            padre[i] = -1;
            G->setMark(i, NOVISITADO);
        ...
        for (int i = 0; i < n; i++) {
            ...
            for (int j = 1; j <= n; j++) {
                if (!G->getMark(j) && dist[j] < minDist) {
                    minDist = dist[j];
                    v = j;
                }
            }
            
            if (v == -1 || dist[v] == INFINITO) break;
            
            G->setMark(v, VISITADO);
            
            for (int w = G->first(v); w != -1; w = G->next(v, w)) {
                if (!G->getMark(w) && dist[w] > (dist[v] + G->weight(v, w))) {
        ...
        if (DViaje == INFINITO) {
            ...
            int ruta[1000]; //numero grande para evitar errores
            int tamRuta = 0;
            ...
            // Invertir la ruta
            for (int i = 0; i < tamRuta / 2; i++) {
                int temp = ruta[i];
                ruta[i] = ruta[tamRuta - 1 - i];
                ruta[tamRuta - 1 - i] = temp;
            }
            ...
            G->info[mejorConductor - 1].estado = 0;
            G->info[destino - 1].estado = 2;
        }
