...
void turno(struct Juego *juego){
    ...
    printf("Ammo Perforador: %d | Ammo Especial: %d\n", juego->armas.ammo_perforador, juego->armas.ammo_especial);
    ...
    if (accion == 'a' || accion == 'd') {
        ...
        if (nueva_pos != juego->jugador_x) {
            ...
            jugador->x = nueva_pos;
            set_message("Jugador movido");
        }
    } else if (accion == '1'){
        disparar_armas(juego, 0);
        juego->turno++;
    } else if (accion == '2'){
        if (disparar_armas(juego, 1)) {
            juego->turno++;
        }
    } else if (accion == '3'){
        if (disparar_armas(juego, 2)) {
            juego->turno++;
        }
    } else {
        set_message("Accion invalida");
        return;
    }
